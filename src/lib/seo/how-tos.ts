...
export const HOW_TOS: HowToSEO[] = [
  {
    ...
    intro: {
      ...
      fr: 'Un tunnel SSH forwarde un port réseau sur une connexion SSH chiffree. Trois saveurs : Local (`-L`) ramene un port distant sur le Mac, Remote (`-R`) expose un port Mac sur le serveur distant, Dynamic SOCKS5 (`-D`) donne un proxy par app. Ce guide couvre les trois en OpenSSH (intégré a macOS) et visuellement dans l\'UI Tunnels de SSHive, choisissez selon votre workflow. Le résultat est le même : port forwarding sur, ephemere, sans overhead VPN.',
    },
    estimatedMinutes: 5,
    steps: [
      {
        ...
        text: {
          ...
          fr: 'Sidebar → clic sur + (nouvelle connexion) ou clic droit sur un profil existant → Modifier. Remplissez hôte, port, username et auth comme d\'habitude, puis cliquez sur "Avance" en bas du dialog. La section Avance se deplie avec trois blocs tunnels listes verticalement : Local (-L), Distant (-R) et SOCKS5 (-D).',
        },
      },
      {
        ...
        text: {
          ...
          fr: 'Sous "Tunnels locaux" cliquez sur + Ajouter. Trois champs apparaissent en ligne : Port local (ex. `5433`, choisissez ≥ 1024 pour éviter sudo), Hôte distant (`db.internal`, ce que le serveur SSH peut atteindre ; mettez `localhost` si la cible tourne sur le bastion), Port distant (`5432`). Sauvegardez. Le tunnel se monte automatiquement a chaque connexion sur ce profil. SSHive bind sur 127.0.0.1 par défaut, pas de champ bind-address expose, c\'est un choix securite. Free : 1 tunnel Local max. Pro : 10.',
        },
      },
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui, les tunnels vivent avec la connexion SSH. Dans SSHive, activez l\'auto-reconnect sur le profil et les tunnels se rattachent automatiquement quand la connexion revient. Pour les tunnels longue durée, reglez `ServerAliveInterval 60` en OpenSSH ou utilisez le keepalive intégré de SSHive (défaut : 30s).',
        },
      },
      {
  ...
  },
  {
    ...
    h1: { en: 'How to generate and use SSH keys on a Mac', fr: 'Comment générer et utiliser des clés SSH sur Mac' },
    hero: {
    ...
    steps: [
      ...
      {
        name: { en: 'Lock down password auth on the remote', fr: 'Verrouiller l\'auth par mot de passe côté serveur' },
        text: {
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Comment utiliser SFTP sur macOS visuellement, transferts drag & drop, édition distante, opérations en lot. Client SFTP gratuit avec terminal SSH dans la même fenetre.',
    },
    ...
    hero: {
      ...
      fr: 'Transferts SFTP drag-and-drop, édition de fichiers distants, opérations en lot, dans la même fenêtre que votre terminal SSH. Gratuit.',
    },
    intro: {
      ...
      fr: 'SFTP est le côté transfert de fichiers de SSH, même auth, même chiffrement, même connexion. macOS ne livre pas de client SFTP graphique ; le Finder ne le parle pas nativement (malgre ce que disent certains vieux tutos). Les options gratuites historiques : Cyberduck (bon mais fenêtre séparée) ou scp depuis le terminal (fonctionnel mais lent). SSHive intégré SFTP dans la même fenêtre que le terminal SSH, drag, drop, edit, save. Ce guide vous montre comment.',
    },
    estimatedMinutes: 3,
    steps: [
      {
        name: { en: 'Open the SFTP panel next to a live SSH terminal', fr: 'Ouvrir le panneau SFTP a côté du terminal SSH actif' },
        text: {
          ...
          fr: 'Connectez-vous a un profil (le terminal SSH s\'ouvre). Dans la toolbar du terminal, cliquez sur l\'**icone dossier** a droite, ou tapez **Cmd+Shift+F**. Le panneau SFTP apparait a côté du terminal en split view. Le bouton toggle layout dans la même toolbar bascule entre horizontal (SFTP a gauche, terminal a droite) et vertical (SFTP en haut, terminal en bas). Le divider est draggable ; double-clic dessus pour reset le ratio. Pour fermer : × dans la barre de nav SFTP ou re-toggler depuis la toolbar.',
        },
      },
      {
        ...
        text: {
          ...
          fr: 'Glissez un fichier ou un **dossier entier** depuis le Finder sur le panneau SFTP, l\'overlay de drop s\'allume et SSHive upload de facon recursive. Les multi-selections marchent aussi. Si un fichier existe déjà côté distant, un dialog propose trois choix : **Remplacer / Dupliquer** (suffixe `(2)`) **/ Ignorer**. En Free, chaque fichier est plafonne a 10 MB (le check passe par `LicenseService.check(\'sftp.upload\')` côté main process avant le transfert). Pro : illimite par fichier. Le download est illimite dans les deux tiers.',
        },
      },
      {
        name: { en: 'Edit text files in place, CodeMirror, Cmd+S, done', fr: 'Éditer des fichiers texte en place, CodeMirror, Cmd+S, c\'est fait' },
        text: {
          ...
          fr: 'Double-clic sur n\'importe quel fichier texte dans le panneau SFTP. Il s\'ouvre directement dans l\'éditeur **CodeMirror 6** intégré, coloration auto pour JS/TS, Python, JSON, HTML, CSS, XML, YAML et Markdown. Editez et tapez **Cmd+S** : SSHive upload directement vers le serveur, pas de fichier temp sur disque, pas de re-upload manuel. Le titre de l\'onglet affiche un point "modifie" tant que pas sauvegarde. Limite pratique autour de 10 MB par fichier dans l\'editeur.',
        },
      },
      {
        name: { en: 'Binary files (images, PDFs, archives), download, edit, push back', fr: 'Fichiers binaires (images, PDF, archives), télécharger, éditer, repousser' },
        text: {
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Clic droit sur n\'importe quel dossier → **Ajouter aux favoris**. Le chemin apparait dans le dropdown favoris du panneau SFTP, `/var/log`, `/etc/nginx`, `~/.config` sont de bons candidats. Les favoris sont par profil, illimités en Free et en Pro, et persistes dans `profiles.json` donc ils survivent aux redemarrages.',
        },
      },
    ],
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. SFTP fait partie du tier gratuit avec le terminal SSH. Le gratuit a 2 sessions concurrentes et 5 profils sauvegardes, pas de restriction spécifique SFTP.',
        },
      },
      {
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Connectez-vous a des bureaux ou serveurs Windows depuis macOS via RDP, client SSHive intégré (pas de jonglage de fenêtres avec Microsoft Remote Desktop), mapping clavier, presse-papiers.',
    },
    h1: { en: 'How to RDP from a Mac to a Windows host', fr: 'Comment faire du RDP depuis Mac vers un hôte Windows' },
    hero: {
      ...
      fr: 'Client RDP intégré dans macOS, pas de switch de fenêtre vers Microsoft Remote Desktop, mapping clavier complet, chiffré via SSH si besoin.',
    },
    intro: {
      ...
      fr: 'RDP (Remote Desktop Protocol) est le protocole d\'accès distant graphique de Microsoft. Depuis Mac, l\'outil evident est Microsoft Remote Desktop, marche bien, mais vit dans sa propre fenêtre loin de votre terminal et panneau SFTP. SSHive inclut un client RDP intégré (powered by IronRDP, une implementation Rust du protocole) qui ouvre les sessions RDP en onglets a côté du SSH/SFTP, une fenêtre pour tout.',
    },
    estimatedMinutes: 4,
    steps: [
      {
        name: { en: 'Enable RDP on the Windows host', fr: 'Activer RDP sur l\'hôte Windows' },
        text: {
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Sidebar → +. Le dialog de connexion a un **selecteur de type** en haut : SSH | RDP | VNC | Local | Outils. Choisissez **RDP**, le port bascule a `3389` automatiquement. Remplissez Nom, Hôte, Username, Mot de passe (avec la checkbox "Sauvegarder" pour le mettre dans le Trousseau macOS), et le champ **Domain** optionnel pour Active Directory (placeholder `DOMAIN`). RDP est **Pro uniquement** : en Free, choisir RDP declenche la modal d\'upgrade au save, la requête n\'atteint jamais `LicenseService.check(\'session.rdp\')`.',
        },
      },
      {
        name: { en: 'Pick a Quality preset and a Resolution, that\'s it', fr: 'Choisir un preset Qualité et une Résolution, c\'est tout' },
        text: {
          ...
          fr: 'Deux boutons visibles : **Qualité** = Auto (défaut, ajuste dynamiquement selon la latence inter-frame) / High (95% / 60fps) / Balanced (80% / 60fps) / Low (50% / 15fps). **Résolution** = Auto (suit la fenêtre SSHive) / 1280×720 / 1366×768 / 1600×900 / 1920×1080 / 2560×1440 / 3840×2160. La compression (RDP6 + bulk + JPEG via libjpeg-turbo SIMD sur les builds Mac App Store), le cache bitmap, le clipboard bidirectionnel (CLIPRDR) et l\'I/O audio sont **actives par défaut et non exposes dans la UI**, laissez Auto sauf raison mesuree.',
        },
      },
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Le RDP direct vers une VM Windows avec IP publique ou joignable via VPN : oui. Les connexions a broker (genre handshake broker d\'Azure Virtual Desktop) sont hors scope, utilisez le client éditeur correspondant pour ceux-la.',
        },
      },
      {
        question: { en: 'Can I copy-paste files between Mac and the Windows host?', fr: 'Puis-je copier-coller des fichiers entre Mac et l\'hôte Windows ?' },
        answer: {
          ...
          fr: 'Le RDP de SSHive supporte la sync de presse-papiers texte. Pour les fichiers, le plus simple : activez OpenSSH sur l\'hôte Windows et utilisez le panneau SFTP de SSHive en parallele, souvent plus rapide que la redirection fichier RDP.',
        },
      },
    ],
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Connectez-vous en VNC depuis macOS vers Raspberry Pi, Proxmox, KVM/QEMU, x11vnc ou RealVNC. Viewer noVNC intégré, pas besoin de relais WebSocket separe.',
    },
    ...
    hero: {
      ...
      fr: 'Viewer VNC intégré, support complet du protocole RFB, fonctionne avec TightVNC/RealVNC/x11vnc/noVNC, et pas besoin de relais WebSocket separe.',
    },
    intro: {
      ...
      fr: 'VNC est le protocole bureau distant cross-platform que tout le monde utilise sur Linux. Que vous geriez le bureau d\'un Raspberry Pi, la console noVNC d\'une VM Proxmox ou une box Linux headless que vous voulez occasionnellement voir graphiquement, VNC est la reponse. Le viewer VNC intégré de SSHive utilise noVNC sous le capot avec un proxy WebSocket intégré, vous vous connectez directement a un serveur VNC TCP sans configurer websockify ou un relais separe.',
    },
    estimatedMinutes: 4,
    steps: [
      {
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Sidebar → +. Dans le selecteur de type en haut, choisissez **VNC**, le port bascule a `5900`. Remplissez Nom, Hôte, Port, Mot de passe. C\'est tout le formulaire VNC, pas de domain, pas de preset qualité, pas de dropdown résolution comme RDP. VNC est **Pro uniquement** : en Free, choisir VNC declenche la modal d\'upgrade au save (le check passe par `LicenseService.check(\'session.vnc\')`). Tous les boutons protocole (encodings, qualité, compression) sont negocies automatiquement.',
        },
      },
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui, le vncserver d\'Apple parle RFB, port 5900. Activez sur le Mac cible : Réglages Système → Général → Partage → Partage d\'ecran. SSHive peut s\'y connecter, bien que pour Mac-Mac, le client intégré soit plus riche (audio, drag-drop).',
        },
      },
    ],
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Configurez les bastions SSH ProxyJump visuellement sur macOS, sans édition de ~/.ssh/config, agent forwarding, chaines multi-saut. Pas-a-pas avec SSHive.',
    },
    ...
    hero: {
      ...
      fr: 'Configuration ProxyJump visuelle, agent forwarding, chaines multi-saut, sans éditer ~/.ssh/config a chaque fois.',
    },
    intro: {
      ...
      fr: 'Un jump host (bastion) est un serveur SSH expose a Internet via lequel vous atteignez des hôtes prives. Configurez un bastion une fois, tous les autres serveurs sont joignables par lui. La manière OpenSSH : `ssh -J bastion target` ou `ProxyJump bastion` dans `~/.ssh/config`. SSHive le rend visuel : configurez le bastion en profil, puis taggez n\'importe quel autre profil avec "Jump Host : bastion", pas d\'édition de config.',
    },
    estimatedMinutes: 3,
    steps: [
      {
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Créez un nouveau profil SSH pour la cible. Remplissez son adresse **privée** (vue depuis le réseau interne du bastion) et l\'auth que la cible accepte. Cliquez **Avance** → bloc **Jump Chain**. Cliquez **+ Ajouter un saut**, un `<select>` apparait listant tous vos autres profils SSH (filtres sur type = ssh, excluant le profil courant). Choisissez le bastion. La chaine se visualise en bas : `bastion → cible`. Chaque saut a des boutons monter/descendre/supprimer. Sauvegardez. Jump Host est **Pro uniquement** (`LicenseService.check(\'jumphost\')`) ; en Free le bloc entier est grise avec le badge PRO.',
        },
      },
      {
        ...
        text: {
          ...
          fr: 'Contrairement a OpenSSH `ProxyJump` (qui a besoin d\'un agent ou de clés correspondantes pour chainer l\'auth), chaque profil dans la chaine SSHive a sa propre auth, mot de passe / clé privée / agent / passphrase, et SSHive utilise l\'auth de chaque profil en parcourant la chaine. Bastion en ed25519, cible en agent-only ? Aucun probleme. La checkbox Forward Agent dans Avance est un toggle a part : activez-la sur le bastion uniquement si un outil *tiers* sur le bastion a besoin de votre agent local (ex. `git push` depuis le bastion avec la clé de votre laptop).',
        },
      },
      {
        ...
        text: {
          ...
          fr: 'Besoin de chainer plus d\'un bastion ? Créez le profil A (pas de saut), le profil B avec Jump Chain `[A]`, le profil C avec Jump Chain `[A, B]`, l\'ordre compte. L\'éditeur de chaine accepte jusqu\'a 5 sauts. SFTP et tunnels sur le profil final marchent de facon transparente a travers chaque saut de la chaine.',
        },
      },
    ],
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui, SFTP et tunnels marchent de facon transparente a travers un jump host. Glissez un fichier sur le panneau SFTP de l\'hôte cible, le fichier passe chiffré : Mac → bastion → cible. Les tunnels marchent pareil.',
        },
      },
    ],
    ...
    steps: [
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'La sync est **a sens unique** : import-only. Le fichier original n\'est pas touche, `ssh hostname` depuis Terminal marche toujours. Si vous modifiez `~/.ssh/config` plus tard, relancez l\'import, les doublons seront skip, seules les nouvelles entrées atterrissent. Note Free : les imports respectent le plafond de 5 profils. Si `~/.ssh/config` a 12 hôtes et que vous êtes en Free, seuls les 5 premiers sont sauvegardes (`ProfileService` tronque a la limite), passez en Pro avant l\'import si vous les voulez tous.',
        },
      },
    ],
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Pas pour le moment, SSHive ne reecrit pas dans `~/.ssh/config`. Les utilisateurs Pro ont un export chiffré du bundle de profils pour backup et partage en équipe, mais c\'est un fichier au format SSHive, pas une config OpenSSH.',
        },
      },
    ],
    relatedFeatures: ['ssh'],
  ...
  },
  {
    ...
    hero: {
      ...
      fr: 'Routez le trafic navigateur via un serveur distant, chiffré, intouchable par les réseaux d\'hotel. Gratuit, sans service VPN.',
    },
    intro: {
      ...
      fr: 'Le flag `-D` de SSH transforme la connexion en proxy SOCKS5. Votre navigateur/app envoie le trafic a `localhost:1080`, SSH le forwarde via le tunnel chiffré vers le serveur distant, et le serveur distant fait la requête sortante reelle. Résultat : le trafic web semble venir de l\'IP du serveur distant, chiffré de Mac jusqu\'au endpoint SSH. Pas besoin de service VPN payant si vous avez un serveur SSH-accessible (serveur perso, VPS a 5 $, même un Raspberry Pi a la maison).',
    },
    estimatedMinutes: 3,
    steps: [
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Pour le trafic navigateur, oui, le tunnel SSH chiffré tout entre Mac et serveur distant. Le réseau d\'hotel ne voit qu\'un stream SSH chiffre. Le trafic sortant du serveur distant utilise encore HTTPS (ou pas), donc SSH seul ne garantit pas le chiffrement bout-en-bout vers chaque destination web. Utilisez aussi les réglages HTTPS-only du navigateur.',
        },
      },
      {
        ...
        answer: {
          ...
          fr: 'SOCKS5 est par app (navigateur, client IRC, etc.) ; un VPN est system-wide. SOCKS5 est gratuit si vous avez un serveur SSH ; un service VPN a un coût d\'abonnement. SOCKS5 gère mal le trafic UDP ; un VPN si. Pour la navigation, SOCKS5 suffit ; pour tout-route (gaming, streaming, trafic app), utilisez WireGuard ou Tailscale.',
        },
      },
    ],
    relatedFeatures: ['tunnels', 'ssh'],
    ...
    steps: [
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Cliquez chaque onglet terminal pour inspecter la sortie côté-a-côté, le serveur qui n\'est pas d\'accord est celui a investiguer. Cmd+Shift+B a nouveau (ou Echap) pour fermer la barre. Les touches retournent dans le terminal individuel que vous focus.',
        },
      },
    ],
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. Dans le panneau de sessions, cochez les sessions a inclure. Cmd+Shift+B cible alors uniquement les sessions cochees. Sauvegardez les groupes courants (ex. "tous web servers") pour sélection en un clic.',
        },
      },
      {
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Laissez Claude ou Cursor exécuter des commandes et lire des fichiers sur vos serveurs SSH distants via le serveur MCP intégré de SSHive. Setup en un toggle, config copier-coller, local et securise.',
    },
    ...
    hero: {
      ...
      fr: 'Laissez votre assistant IA exécuter des commandes, lire des fichiers et trier les incidents directement sur vos serveurs, via le serveur MCP intégré de SSHive.',
    },
    intro: {
      ...
      fr: 'MCP (Model Context Protocol) est le standard ouvert introduit par Anthropic pour que les assistants IA parlent aux outils locaux. SSHive embarque un serveur MCP intégré, Claude Code, Cursor ou Claude Desktop peuvent lire vos sessions SSH actives, lancer des commandes et parcourir des filesystems SFTP. Le serveur n\'ecoute que sur localhost, utilise une auth Bearer token stockée dans le Trousseau macOS et herite des permissions de vos connexions SSH existantes. Setup en 30 secondes.',
    },
    estimatedMinutes: 3,
    steps: [
      {
      ...
      },
      {
        ...
        text: {
          ...
          fr: 'Pour **Claude Code** (CLI + extension IDE) et **Cursor** : SSHive ecrit directement dans `~/.claude.json` et `~/.cursor/mcp.json` (mode `0600`, le token est sensible). La page MCP affiche leur statut `Configure ✓` ou `Non detecte`. Vous n\'avez rien a copier. Pour **Claude Desktop** (qui ne supporte que stdio, pas HTTP nativement) : SSHive affiche un bloc copiable séparé utilisant `npx -y mcp-remote` comme shim stdio-vers-HTTP, collez-le dans `claude_desktop_config.json`. Pour tout autre client compatible MCP : un bouton "Copier la config Streamable HTTP" copie un bloc JSON **avec votre vrai token** pret a coller n\'importe ou.',
        },
      },
      {
