...
export const IOS_PAGES: IosFeatureSEO[] = [
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    metaDescription: {
      ...
      fr: 'Terminal SSH xterm-256color complet sur iPhone et iPad. Clés Ed25519 et ECDSA avec passphrase, Trousseau Touch ID / Face ID, 8 themes et tunnels SSH. Gratuit pour commencer.',
    },
    h1: {
    ...
    intro: {
      ...
      fr: 'Apple n\'a jamais livre de client SSH. Si l\'alerte prod tombe a 2h alors que vous etes loin d\'un laptop, l\'iPhone par defaut ne vous donne rien, pas de `ssh user@host`, pas de terminal, pas de SFTP. SSHive comble ce manque avec une app iPhone et iPad native : un vrai terminal xterm-256color base sur SwiftTerm, auth par clé privee Ed25519 + ECDSA P-256 avec passphrase bcrypt-pbkdf, tunnels locaux, et credentials stockes dans le Trousseau iOS derriere Face ID ou Touch ID, le tout dans une interface SwiftUI qui s\'adapte au TabView iPhone et au NavigationSplitView iPad.\n\nLe modèle est volontairement simple : pas d\'abonnement, pas de données qui quittent votre appareil, achat Pro unique, a vous pour la vie. Le tier gratuit couvre 2 sessions simultanees, 5 profils sauvegardes, 1 tunnel local, 3 snippets et 10 Mo par fichier SFTP. Pro a 9,99 € unique sur l\'App Store (Universal Purchase : payez une fois, utilisez sur iPhone, iPad et Mac). Si quelque chose vous semble manquer ou ameliorable, on ecoute, ecrivez-nous.',
    },
    capabilities: [
      {
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Importez vos clés privees existantes au format OpenSSH (Ed25519 et ECDSA P-256). Les clés passphrase sont dechiffrees sur l\'appareil avec bcrypt-pbkdf + AES-256-CTR. Deux modes : "memoriser passphrase derriere Face ID / Touch ID" ou "demander a chaque connexion". Vérification known-hosts (TOFU) avec empreintes SHA-256 pour detecter le MITM.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Free couvre 2 sessions SSH simultanees ; Pro illimite. Glissez entre elles sur iPhone ou split-view sur iPad. Les sessions survivent au backgrounding quand iOS le permet ; reconnexion auto sur changement réseau, keep-alive configurable (5 a 120 s) maintient les sessions inactives vivantes.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Tapez une fois, executez sur N serveurs. Selectionnez plusieurs sessions connectées et la saisie clavier est mirroir sur toutes, parfait pour les updates de flotte (`apt update && apt upgrade -y` sur 8 hotes en même temps) ou des health checks rapides sur un tier. Feature Pro.',
        },
      },
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. Clés privees Ed25519 et ECDSA P-256 au format OpenSSH, y compris les clés passphrase (bcrypt-pbkdf + AES-256-CTR). Vous choisissez entre "memoriser passphrase derriere Face ID / Touch ID" ou "demander a chaque connexion" par cle.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Oui, par defaut rien ne quitte votre appareil : les credentials sont dans le Trousseau iOS local avec `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`. La sync iCloud des profils est opt-in (Pro) et passe par votre propre base CloudKit privee, credentials chiffres de bout en bout ; laissez-la coupee et utilisez l\'export/import `.sshive` pour deplacer les profils manuellement.',
        },
      },
      {
        question: {
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    intro: {
      ...
      fr: 'Lire et écrire des fichiers sur un serveur distant depuis un iPhone est plus penible que ca devrait, la plupart des workflows finissent en copier-coller dans un terminal SSH, ou avec un gestionnaire de fichiers web ouvert dans un onglet. SSHive embarque un vrai client SFTP natif sur iOS et iPadOS : un explorateur de fichiers distants, une file de transferts avec progression temps reel, un editeur de texte integre pour les edits sur place, et une intégration avec l\'app Files iOS pour glisser un fichier d\'iCloud Drive directement sur un serveur. Les credentials et host keys sont dans le Trousseau derriere Face ID ou Touch ID, même modèle de sécurité que le terminal SSH.\n\nFree couvre 10 Mo par fichier (suffisant pour la plupart des configs, scripts et petits assets), Pro debloque la taille illimitee et ajoute le logging de session et l\'export/import `.sshive`. Universal Purchase : payez Pro une fois sur l\'App Store, utilisez sur iPhone, iPad et Mac. Pas d\'abonnement, pas de données envoyees vers nous, et si vous tombez sur un cas d\'usage que l\'editeur ou le browser ne couvrent pas bien, dites-le-nous, on ecoute.',
    },
    capabilities: [
      {
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Tap sur un `.conf`, `.yml`, `.json`, `.env` ou n\'importe quel fichier texte pour l\'ouvrir dans l\'editeur de SSHive. Coloration syntaxique, numéros de ligne, recherche-remplacement, Tab/Shift+Tab indent. Save reecrit via SFTP. Parfait pour les 90 % d\'edits distants qui sont "corriger une ligne dans nginx.conf et reload".',
        },
      },
      {
    ...
    why: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Pour le workflow distant le plus courant, "corriger une ligne dans une config et reload le service", l\'editeur integre de SSHive evite le download, edit, reupload. Tap, editez, sauvegardez. SFTP gère l\'aller-retour.',
        },
      },
      {
    ...
    competitors: [
      {
        ...
        pitch: {
          ...
          fr: 'Un gestionnaire de fichiers polyvalent qui inclut le SFTP. Très bien si le SFTP est un outil parmi d\'autres dont vous avez besoin (cloud drives, ZIP, downloads). Pour le SFTP comme usage principal, l\'editeur et les clés par profil de SSHive sont en général plus pratiques.',
        },
    ...
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Oui. Ouvrez le profil SFTP, tap sur nginx.conf, l\'editeur integre s\'ouvre avec coloration et numéros de ligne. Editez, tap Save, SFTP reecrit sur le serveur. Ouvrez une session SSH parallele dans la même fenêtre SSHive pour `nginx -t && nginx -s reload`.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Oui. SFTP est un sous-protocole de SSH, chaque octet passe par le tunnel chiffre SSH. SSHive supporte les ciphers SSH modernes : AES-256-GCM, chacha20-poly1305, avec vérification de host-key Ed25519 et ECDSA.',
        },
      },
      {
        question: {
    ...
    capabilities: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Defaut a 1920×1080 avec scaling adaptatif pour l\'écran iPhone ou iPad. Surchargeable par profil pour matcher exactement la résolution attendue par le serveur (utile quand on connecte depuis un petit iPhone et qu\'on veut que l\'écran distant rende en taille iPad cote serveur).',
        },
      },
      {
    ...
    why: [
      {
        ...
        body: {
          ...
          fr: 'Un champ Domaine dedie, separe du username, qui accepte NetBIOS (`CORP`) et FQDN (`corp.example.com`). NLA + NTLMv2 negocies par defaut. Même modèle d\'entrée que les admins AD utilisent déjà sur Windows, pas besoin d\'empaqueter `DOMAIN\\username`.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Sur Microsoft Remote Desktop vous faites du RDP. Pour avoir SSH ou SFTP a cote, vous passez a une autre app et perdez le contexte VPN. SSHive garde RDP, SSH, SFTP, VNC et VPN dans une fenêtre, exactement ce qu\'il faut a un on-call a 2h du matin.',
        },
      },
      {
    ...
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Oui, case par profil pour desactiver NLA. Utile pour les vieux Windows Server 2008 R2 ou environnements mal configures. Par defaut NLA est active, ce que les systèmes modernes attendent.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Une barre expose les touches que les écrans tactiles ne peuvent pas envoyer (Win, Alt, Ctrl, Esc, F1-F12, fleches). Un bouton dedie Ctrl+Alt+Suppr toujours visible. Avec un Magic Keyboard sur iPad, le clavier complet marche y compris la touche Windows (mappee a Cmd Mac par defaut, inversable par profil).',
        },
      },
    ],
    deepDiveHeading: {
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    intro: {
      ...
      fr: 'Le VNC sur iOS a toujours ete le cas le plus delicat, Apple Screen Sharing n\'existe pas sur iPhone, et beaucoup d\'apps VNC iOS tierces se concentrent sur le RFB plain sans TLS, sans support ARD, sans profils sauvegardes. SSHive embarque RoyalVNC sous le capot (le même moteur utilise par Royal TSX sur Mac), qui supporte toute la famille VNC : auth ARD pour Mac-vers-Mac, VeNCrypt + TLS pour RFB chiffre, et RFB nu pour serveurs legacy, enveloppe dans un tunnel SSH via l\'UI Tunnels de SSHive quand vous avez besoin d\'un chiffrement que le serveur VNC lui-même n\'offre pas.\n\nL\'UI est concue pour le tactile : tap = clic gauche, tap deux doigts = clic droit, pinch pour zoomer, pan deux doigts, et un clavier VNC a l\'écran specifique qui expose touches de fonction, modifiers et fleches qu\'un clavier de telephone n\'a pas. Free inclut 2 sessions VNC simultanees ; Pro retire la limite. Universal Purchase avec macOS et iPad. Pas d\'abonnement, pas de données envoyees vers nous, si un serveur auquel vous vous connectez fait des siennes, dites-le-nous, on regardera.',
    },
    capabilities: [
      {
        ...
        body: {
          ...
          fr: 'Le même moteur VNC que Royal TSX, porte sur iOS et iPadOS. TLS / VeNCrypt complet, auth ARD et RFB standard. Se connecte aux serveurs RealVNC, TigerVNC, libvncserver, X11 vncserver, Partage d\'écran Apple, basiquement n\'importe quel serveur VNC raisonnable.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Tap = clic gauche, long-press = clic droit, tap deux doigts = clic du milieu. Pinch pour zoomer, pan deux doigts pour scroller un grand bureau. Tap trois doigts pour afficher/masquer le clavier VNC. Le modèle de gestes est coherent avec le reste d\'iOS, pas de memoire musculaire propre a l\'app a reapprendre.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Un clavier dedie a l\'écran expose les touches qu\'un clavier soft iOS ne peut pas envoyer : F1-F12, Esc, Tab, Ctrl, Alt, fleches, Impr écran, Inser. Combine a la saisie texte native iPhone/iPad, vous pouvez utiliser presque n\'importe quel bureau Linux/macOS sans clavier hardware.',
        },
      },
      {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. ARD est un des modes d\'auth supportes par RoyalVNC. Se connecter a un Mac avec Partage d\'écran active fonctionne sans config supplementaire, y compris avec le username et password ARD.',
        },
      },
      {
        question: {
    ...
    capabilities: [
      {
        ...
        body: {
          ...
          fr: 'Les trois protocoles VPN les plus deployes supportes nativement : IKEv2 (le defaut moderne sur macOS/iOS), IPSec avec Xauth (encore courant dans les gateways corporate), et OpenVPN (via import de config `.ovpn`, les mêmes fichiers qu\'OpenVPN Connect accepte).',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Base sur le framework NetworkExtension / Packet Tunnel Provider d\'Apple, les mêmes APIs que Cisco AnyConnect et OpenVPN Connect. Le VPN est donc reel (routage L3 complet, pas un hack proxy SOCKS), respecte la gestion de batterie iOS, et apparait dans Réglages > VPN comme n\'importe quel profil VPN.',
        },
      },
      {
    ...
    why: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'SSHive est payante (Pro 9,99 € unique) et n\'a pas de modèle publicitaire, donc l\'app n\'a tout simplement pas besoin de regarder votre trafic VPN. Pas de telemetrie, pas d\'analytics tiers, pas de trafic relaye par des serveurs qu\'on contrôle, le VPN va directement de votre iPhone a votre endpoint configure.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Le VPN de SSHive utilise NetworkExtension comme les autres apps VPN systeme. Si vous utilisez déjà Tailscale ou WireGuard pour des tailnets personnels, continuez, ils tournent a cote du VPN SSHive au niveau systeme. Le VPN SSHive couvre les cas IKEv2/IPSec/OpenVPN que Tailscale et WireGuard ne couvrent pas.',
        },
      },
    ],
    competitorsHeading: {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. AirDrop ou partagez le fichier `.ovpn` dans SSHive, le parser gère les blocs inline `<ca>`, `<cert>`, `<key>`, TLS-auth et les variantes courantes. Username/password (ou passphrase de certificat client) sont ensuite demandes a la première connexion et stockes dans le Trousseau iOS.',
        },
      },
      {
        question: {
