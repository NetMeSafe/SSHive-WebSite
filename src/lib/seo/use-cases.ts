  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: {
          en: 'SSHive ships with a local MCP (Model Context Protocol) server, listening on a Bearer-token-protected localhost port. Once enabled, Claude Code, Cursor and Claude Desktop are auto-configured: they can call `ssh_execute` on your active SSH sessions, browse remote files via `sftp_list`, and read/write files via `sftp_read_file` / `sftp_write_file` / `sftp_write_chunk`. Translation: ask your AI assistant "tail the last 200 lines of nginx error log on prod" and it does it through your SSHive session, no extra setup.',
          fr: 'SSHive embarque un serveur MCP (Model Context Protocol) local, ecoutant sur un port localhost protege par Bearer token. Une fois active, Claude Code, Cursor et Claude Desktop sont auto-configures : ils peuvent appeler `ssh_execute` sur vos sessions SSH actives, parcourir les fichiers distants via `sftp_list`, et lire/ecrire des fichiers via `sftp_read_file` / `sftp_write_file` / `sftp_write_chunk`. Traduction : demandez a votre assistant IA "tail les 200 dernieres lignes du log d\'erreur nginx sur prod" et il le fait via votre session SSHive, sans setup supplementaire.',
        },
      },
    ],
    faq: [
