...
> SSHive is a native SSH, SFTP, RDP, VNC and VPN client for macOS, iPhone and iPad, which on the Mac also speaks FTP, FTPS, Telnet and RS-232 serial. Two things set it apart: a serial console and a Telnet client that work inside the Mac App Store sandbox, covering the console-cable and legacy-equipment work that usually sends Mac users to PuTTY; and a built-in MCP (Model Context Protocol) server that lets AI assistants such as Claude Code, Cursor and Claude Desktop drive SSH sessions and SFTP transfers directly, with no extra install. Freemium: free on the App Store, SSHive Pro is a one-time purchase (no subscription, Universal Purchase: Mac, iPhone, iPad).
...
- macOS-only: serial console over a USB-to-serial adapter (1200-230400 baud, 8-N-1 and every other framing, RTS/CTS and XON/XOFF, BREAK signal; works in the Mac App Store build via com.apple.security.device.serial), Telnet client (RFC 854, ECHO/SGA/NAWS negotiation), FTP and explicit FTPS (AUTH TLS) in the same dual-pane browser as SFTP, the local MCP server for AI assistants, broadcast mode (run a command on every open session), remote (-R) and SOCKS5 (-D) tunnels, jump hosts, TOTP/HOTP authenticator, shared accounts, session logging. iOS/iPadOS-only: VPN client (IKEv2, IPSec/Xauth, OpenVPN).
