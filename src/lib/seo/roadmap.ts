...
export const ROADMAP: RoadmapItem[] = [
  {
    ...
    title: {
      ...
      fr: 'Correction : Pro repasse parfois en Free après une mise a jour',
    },
    body: {
  ...
  },
  {
    ...
    body: {
      ...
      fr: 'Livre. Activez la sync iCloud (Pro, opt-in) et vos profils de connexion vous suivent : ajoutez un hôte sur le Mac, il apparait sur iPhone et iPad en quelques secondes, avec les mêmes tags, notes et definitions de tunnels. Tout passe par votre propre base iCloud privée (CloudKit), pas de compte SSHive, aucun serveur a nous dans la boucle, et vous pouvez couper la sync par appareil a tout moment.\n\nLes profils et comptes partages se synchronisent en metadonnees ; les credentials transitent par un canal séparé chiffré de bout en bout, donc ce qui quitte un appareil est illisible pour quiconque sauf vos propres appareils. Si vous preferez tout garder strictement local, laissez le toggle coupe et utilisez l\'export chiffré .sshive.',
    },
    platforms: ['mac', 'iphone', 'ipad'],
  },
  {
    ...
    title: {
      ...
      fr: 'Support YubiKey / clé hardware via NFC',
    },
    body: {
      ...
      fr: 'Pour les équipes qui utilisent déjà un YubiKey 5 NFC pour SSH, la piece manquante sur iPhone, c\'est de lire la clé via la stack Core NFC de l\'iPhone et de l\'utiliser comme SSH agent. Le plan : supporter les clés SSH FIDO2/PIV (`ssh-ed25519-sk`, `ecdsa-sk`) pour qu\'un iPhone puisse s\'authentifier sur un serveur qui exige une clé hardware, avec une confirmation Face ID côté SSHive et un tap NFC sur le YubiKey.\n\nCa ferme le trou ou l\'iPhone est aujourd\'hui le maillon faible des politiques SSH 2FA. iPad garde le support YubiKey Lightning / USB-C via le port USB-C ; Mac utilise déjà le YubiKey via l\'intégration agent normale de SSHive.',
    },
  ...
  },
  {
    ...
    title: {
      ...
      fr: 'Meilleur Stage Manager et multi-fenêtres sur iPad',
    },
    body: {
      ...
      fr: 'L\'iPad Pro avec Magic Keyboard est proche d\'un vrai laptop, mais le layout iPad actuel de SSHive traite encore la fenêtre comme un espace unique. Le plan : supporter le vrai multi-fenêtres, ouvrir la session SSH prod dans une fenêtre Stage Manager, le RDP staging dans une autre, un browser SFTP dans une troisième, tout a côté de Notes ou Safari. Drag-and-drop entre fenêtres SSHive pour les transferts SFTP, raccourcis clavier Magic Keyboard natifs (Cmd+T nouvelle tab, Cmd+W fermer, Cmd+1..9 switcher).\n\nL\'objectif : que l\'iPad Pro avec Magic Keyboard devienne une station de travail principale confortable pour le travail distant, pas juste un appareil de secours.',
    },
    platforms: ['ipad'],
  },
];
