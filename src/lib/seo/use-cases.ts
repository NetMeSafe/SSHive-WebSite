...
export const USE_CASES: UseCaseSEO[] = [
  {
    ...
    metaDescription: {
      ...
      fr: 'Connectez-vous a vos serveurs depuis iPhone ou iPad avec SSHive, terminal SSH iOS natif et gestionnaire SFTP, profils synchronises depuis le Mac, Trousseau sécurisé par Touch ID ou Face ID.',
    },
    h1: {
    ...
    hero: {
      ...
      fr: 'Un client SSH Apple natif qui vous suit partout, terminal et SFTP sur iOS, suite tout-en-un complète sur Mac.',
    },
    intro: {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'SSHive est sur l\'App Store avec un tier gratuit et une mise a niveau Pro, voir la page Tarifs pour les details a jour. Le tier gratuit couvre SSH et SFTP pour usage individuel ; Pro etend les limites et debloque des fonctionnalités selon la plateforme.',
        },
      },
      {
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Oui, Magic Keyboard, Smart Keyboard Folio et claviers Bluetooth sont tous geres. Les raccourcis courants (Cmd+C/V, Cmd+T pour nouvel onglet, fleches, touches de fonction) se traduisent correctement. Le bandeau de modificateurs a l\'écran est masque quand un clavier externe est connecte.',
        },
      },
    ],
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Gérez votre Raspberry Pi depuis macOS avec SSHive : terminal SSH, transferts SFTP, bureau VNC, tunnels SSH. Client macOS natif gratuit. Pas besoin de PuTTY.',
    },
    h1: {
    ...
    hero: {
      ...
      fr: 'Tout ce qu\'il faut pour gérer un Pi depuis macOS, SSH, SFTP, VNC et tunnels, dans une seule app native.',
    },
    intro: {
  ...
  },
  {
    ...
    h1: {
      ...
      fr: 'Gérer vos instances AWS EC2 depuis votre Mac',
    },
    hero: {
    ...
    faq: [
      ...
      {
        question: { en: 'How does SSHive handle AWS instance IP changes?', fr: 'Comment SSHive gère les changements d\'IP d\'instances AWS ?' },
        answer: {
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Gérez votre home lab depuis Mac : SSH, transfert SFTP, VNC, tunnels SSH pour services self-hosted (Plex, Jellyfin, Home Assistant, Nextcloud).',
    },
    h1: {
      en: 'Manage your home server from your Mac',
      fr: 'Gérer votre serveur perso depuis votre Mac',
    },
    hero: {
    ...
    sections: [
      ...
      {
        heading: { en: 'Backup with SFTP from one window', fr: 'Backup en SFTP depuis une seule fenêtre' },
        body: {
    ...
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Non, SSHive tourne sur SSH/TCP standard, pas le protocole UDP de Mosh. Pour des liens très instables (tethering mobile, satellite), Mosh a du roaming et de l\'echo predictif que SSHive n\'a pas. L\'auto-reconnect de SSHive aide pour les chutes intermittentes mais n\'est pas un remplacement Mosh.',
        },
      },
    ],
  ...
  },
  {
    ...
    metaTitle: {
      ...
      fr: 'Gérer Proxmox depuis Mac, SSH, console VNC, SFTP',
    },
    metaDescription: {
    ...
    hero: {
      ...
      fr: 'SSH vers les noeuds PVE, console noVNC vers les VMs invitees, SFTP pour les ISOs, workflows Proxmox dans une fenêtre macOS native.',
    },
    intro: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Proxmox embarque noVNC pour l\'accès console aux invites, le même protocole que parle le viewer VNC de SSHive. Pour ouvrir une console invite hors de l\'UI web PVE : dans l\'UI, allez sur la VM, Console → "expand", notez le pattern d\'URL. Ou plus simple : requetez `qm vncproxy <vmid>` en SSH pour avoir un port et mot de passe one-time, puis connectez depuis le panneau VNC de SSHive vers `pve-host:port`. Utile pour les VMs sans connectivite réseau encore (installs frais, boots rescue).',
        },
      },
      {
    ...
    faq: [
      ...
      {
        question: { en: 'How do I securely access PVE web UI from outside my LAN?', fr: 'Comment acceder a l\'UI web PVE depuis l\'exterieur du LAN en sécurité ?' },
        answer: {
          ...
          fr: 'N\'exposez pas 8006 sur le routeur. A la place, utilisez la fonctionnalité tunnel de SSHive : dans le profil du noeud PVE, ajoutez un forward Local 8006 → localhost:8006. Connectez en SSH, puis ouvrez `https://localhost:8006` dans Safari/Chrome. Le trafic UI web est maintenant tunnele via SSH.',
        },
      },
    ],
  ...
  },
  {
    ...
    h1: { en: 'Kubernetes ops from your Mac', fr: 'Opérations Kubernetes depuis votre Mac' },
    hero: {
    ...
    sections: [
      {
        ...
        body: {
          ...
          fr: 'Créez le bastion comme profil SSH normal dans SSHive : hostname, port, user, cle. Connectez une fois pour vérifier et accepter la clé hote. Marquez ce profil comme "Bastion / Jump Host" dans les paramètres du profil, il apparait dans le dropdown Jump Host des autres profils. Optionnellement, activez "Agent forwarding" si vous devez vous authentifier aux sauts suivants avec la même cle.',
        },
      },
      {
  ...
  },
  {
    ...
    hero: {
      ...
      fr: 'RDP vers les sessions bureau et SSH vers PowerShell, Windows Server dans une seule fenêtre SSHive.',
    },
    intro: {
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Gérez Synology DSM, QNAP QTS ou tout NAS SSH-compatible depuis macOS : terminal, navigateur SFTP, tunnels pour UI DSM, broadcast sur plusieurs NAS.',
    },
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Sur Ethernet gigabit, le SFTP via SSH sature typiquement le lien (110-120 Mo/s). Le goulot est les IOPS du stockage interne du NAS et le CPU pour le chiffrement SSH. Les NAS a CPU ARM faibles (Synology DS séries anciennes) peinent a depasser 50 Mo/s. Utilisez des ciphers compatibles AES-NI (`aes128-gcm@openssh.com` est rapide).',
        },
      },
      {
        ...
        answer: {
          ...
          fr: 'Non, Synology Drive fait de la sync en arriere-plan et de la résolution de conflits. Le SFTP de SSHive est pour les ops fichier explicites. Utilisez les deux : Drive pour la sync continue, SSHive pour les taches admin ad-hoc.',
        },
      },
    ],
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Vous cherchez un terminal SSH tout-en-un sur macOS qui regroupe SFTP, RDP, VNC, tunnels et intégration IA ? SSHive est le client natif Mac pense pour ce workflow. Téléchargement gratuit.',
    },
    h1: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: {
          en: 'SSHive ships with a local MCP (Model Context Protocol) server on `127.0.0.1:49422`, protected by a Bearer token. Toggle it ON in Settings → MCP and SSHive auto-injects an `mcpServers.sshive` entry into the config files of detected clients (Claude Code, Cursor) and shows a copy-paste block for Claude Desktop. The 11 tools cover `ssh_list_sessions`, `ssh_execute` and full SFTP file management: `sftp_list`, `sftp_read_file`, `sftp_write_file`, `sftp_write_file_chunk`, `sftp_write_from_local_path`, `sftp_download_to_local_path`, `sftp_mkdir`, `sftp_rename`, `sftp_delete`. Translation: ask your AI assistant "tail the last 200 lines of nginx error log on prod" and it does it through your SSHive session, no extra setup. MCP is Pro-only.',
          fr: 'SSHive embarque un serveur MCP (Model Context Protocol) local sur `127.0.0.1:49422`, protège par un Bearer token. Basculez-le ON dans Paramètres → MCP et SSHive auto-injecte une entrée `mcpServers.sshive` dans les fichiers de config des clients detectes (Claude Code, Cursor) et affiche un bloc copier-coller pour Claude Desktop. Les 11 outils couvrent `ssh_list_sessions`, `ssh_execute` et la gestion complète des fichiers SFTP : `sftp_list`, `sftp_read_file`, `sftp_write_file`, `sftp_write_file_chunk`, `sftp_write_from_local_path`, `sftp_download_to_local_path`, `sftp_mkdir`, `sftp_rename`, `sftp_delete`. Traduction : demandez a votre assistant IA "tail les 200 dernières lignes du log d\'erreur nginx sur prod" et il le fait via votre session SSHive, sans setup supplementaire. MCP est Pro uniquement.',
        },
      },
    ],
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'SSHive Free couvre le terminal SSH, le gestionnaire SFTP, les snippets et le stockage Trousseau avec une petite limite de sessions concurrentes. SSHive Pro sur le Mac App Store debloque sessions illimitees, RDP, VNC, tunnels, broadcast et intégration MCP. Le prix et les derniers details des tiers sont sur la page Tarifs.',
        },
      },
    ],
  ...
  },
  {
    ...
    metaDescription: {
      ...
      fr: 'Un workflow SSH moderne pour les développeurs macOS : profils par projet, tunnels DB dev, scripts de deploy en snippets, intégration Claude/Cursor MCP pour ops assistees IA.',
    },
    h1: { en: 'A developer SSH workflow built for macOS', fr: 'Un workflow SSH développeur pense pour macOS' },
    hero: {
    ...
    sections: [
      {
        ...
        body: {
          ...
          fr: 'Créez un dossier par projet. Dedans, profils pour prod, staging, dev. Tag couleur prod en rouge, staging en orange, dev en vert, repere visuel qui empeche de taper dans le mauvais terminal. Sauvegardez les commandes communes (`tail app log`, `restart service`, `db backup`) en snippets project-scoped.',
        },
      },
      {
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Activez le serveur MCP de SSHive. Claude Code ou Cursor dans votre IDE peut maintenant `ssh_execute` sur n\'importe quelle session SSHive ouverte. Demandez "tail les 200 dernières lignes d\'app.log sur staging et trouve les erreurs 500", Claude le fait. Demandez "compare la config nginx staging au repo local", Claude lit via `sftp_read_file`, diffe. Reste hors-terminal pour les taches routinieres.',
        },
      },
    ],
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. SSHive peut importer depuis `~/.ssh/config` et respecter ses directives, mais ne le demande pas. Vous pouvez utiliser le système de profils de SSHive en parallele de `ssh user@host` depuis n\'importe quel autre terminal, pas de conflit.',
        },
      },
      {
