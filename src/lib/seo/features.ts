...
export const FEATURE_SEO: Record<Feature, FeatureSEO> = {
  ...
  mcp: {
    intro: {
      en: 'Model Context Protocol (MCP) is the standard Anthropic introduced in late 2024 for letting AI assistants talk to local tools. SSHive ships with a built-in MCP server — meaning Claude Code, Cursor, and Claude Desktop can read your SSH session list, execute commands on connected hosts, browse SFTP, and read/write remote files. Toggle one switch in SSHive\'s settings, and the MCP server is up and registered in your AI clients automatically — no manual config-editing, no JSON to copy-paste.\n\nThe server exposes 5 tools: `ssh_execute`, `sftp_list`, `sftp_read_file`, `sftp_write_file`, `sftp_write_chunk`. Authentication uses a Bearer token generated on first run, stored in the macOS Keychain. The server only listens on localhost — it never accepts external connections — and every command is gated through your existing SSH connection auth, so the AI cannot do anything you can\'t already do. You can revoke and rotate the token from settings at any time.',
      fr: 'Model Context Protocol (MCP) est le standard introduit par Anthropic fin 2024 pour permettre aux assistants IA de parler aux outils locaux. SSHive embarque un serveur MCP integre — Claude Code, Cursor et Claude Desktop peuvent donc lire votre liste de sessions SSH, executer des commandes sur les hotes connectes, parcourir SFTP et lire/ecrire des fichiers distants. Activez un interrupteur dans les parametres SSHive, et le serveur MCP est en ligne et enregistre dans vos clients IA automatiquement — pas d\'edition de config manuelle, pas de JSON a copier-coller.\n\nLe serveur expose 5 outils : `ssh_execute`, `sftp_list`, `sftp_read_file`, `sftp_write_file`, `sftp_write_chunk`. L\'authentification utilise un Bearer token genere au premier lancement, stocke dans le Trousseau macOS. Le serveur n\'ecoute que sur localhost — il n\'accepte jamais de connexions externes — et chaque commande passe par l\'auth de votre connexion SSH existante, donc l\'IA ne peut rien faire que vous ne puissiez deja faire. Vous pouvez revoquer et tourner le token depuis les parametres a tout moment.',
    },
    ...
    faq: [
      ...
      },
      {
        ...
        answer: {
          en: 'Claude Code (CLI), Cursor (IDE), and Claude Desktop are auto-configured with one click. Any MCP-compliant client works manually — point it at `http://localhost:PORT` with the Bearer token from SSHive settings.',
          fr: 'Claude Code (CLI), Cursor (IDE) et Claude Desktop sont auto-configures en un clic. Tout client conforme MCP fonctionne manuellement — pointez-le sur `http://localhost:PORT` avec le Bearer token des parametres SSHive.',
        },
      },
      {
