...
export const IOS_PAGES: IosFeatureSEO[] = [
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    metaDescription: {
      ...
      fr: 'Terminal SSH xterm-256color complet sur iPhone et iPad. Clés Ed25519 et ECDSA avec passphrase, Trousseau Touch ID / Face ID, 8 thèmes et tunnels SSH. Gratuit pour commencer.',
    },
    h1: {
    ...
    hero: {
      en: 'xterm-256color terminal, Ed25519 keys with passphrase, Touch ID Keychain, tunnels, the full SSH workflow, native on iOS and iPadOS.',
    ...
    capabilities: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Free couvre 2 sessions SSH simultanées ; Pro illimite. Glissez entre elles sur iPhone ou split-view sur iPad. Les sessions survivent au backgrounding quand iOS le permet ; reconnexion auto sur changement réseau, keep-alive configurable (5 a 120 s) maintient les sessions inactives vivantes.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Tapez une fois, executez sur N serveurs. Selectionnez plusieurs sessions connectées et la saisie clavier est mirroir sur toutes, parfait pour les updates de flotte (`apt update && apt upgrade -y` sur 8 hôtes en même temps) ou des health checks rapides sur un tier. Feature Pro.',
        },
      },
      {
      ...
      {
        title: { en: 'SSH keys & known hosts', fr: 'Clés SSH et known hosts' },
        body: {
          en: 'Generate Ed25519 or ECDSA key pairs directly on the device, import existing keys (with or without passphrase), and copy the public key to paste into `authorized_keys`. Host fingerprints are verified and pinned on first connect (known hosts), with a clear prompt if a server key ever changes.',
          fr: 'Generez des paires de clés Ed25519 ou ECDSA directement sur l\'appareil, importez des clés existantes (avec ou sans passphrase), et copiez la clé publique a coller dans `authorized_keys`. Les empreintes des hôtes sont verifiees et epinglees a la première connexion (known hosts), avec une alerte claire si la clé d\'un serveur change.',
        },
      },
    ],
    whyHeading: {
    ...
    why: [
      ...
      },
      {
        ...
        body: {
          en: 'Buy Pro once on the App Store, use it on iPhone, iPad and Mac. The macOS version adds broadcast mode, the OTP authenticator, remote (-R) and SOCKS5 (-D) tunnels, an MCP server for Claude Code / Cursor / Claude Desktop, and the SSH config import from `~/.ssh/config`. The iOS app already ships RDP, VNC, VPN, local tunnels, network tools and snippets.',
          fr: 'Achetez Pro une fois sur l\'App Store, utilisez sur iPhone, iPad et Mac. La version macOS ajoute le mode broadcast, l\'authenticator OTP, les tunnels distants (-R) et SOCKS5 (-D), un serveur MCP pour Claude Code / Cursor / Claude Desktop, et l\'import SSH config depuis `~/.ssh/config`. L\'app iOS embarque déjà RDP, VNC, VPN, tunnels locaux, outils réseau et snippets.',
        },
      },
    ],
    competitorsHeading: {
    ...
    competitors: [
      {
        ...
        pitch: {
          ...
          fr: 'Client cross-platform avec sync cloud entre Mac, iOS, Windows et Linux. Bonne finition et tier gratuit genereux. Les fonctionnalités avancées et la sync d\'équipe sont dans l\'abonnement paye, bon choix si vous voulez tout dans le cloud.',
        },
      ...
      },
      {
        ...
        pitch: {
          ...
          fr: 'Un client SSH iOS premium très apprecie de Panic, avec une UI magnifique. SSH + SFTP uniquement sur iOS, pas de RDP, VNC ou VPN. Très bon choix si vous voulez la meilleure experience terminal possible et rien d\'autre.',
        },
      ...
      },
      {
        ...
        pitch: {
          ...
          fr: 'Un angle différent : surveille les serveurs via Netdata/Prometheus et ajoute SSH par-dessus. Ideal si le monitoring est votre cas d\'usage principal et le shell distant le bonus.',
        },
    ...
    faq: [
      {
        question: {
          ...
          fr: 'SSHive sur iOS supporte-t-il les clés SSH avec passphrase ?',
        },
        answer: {
          ...
          fr: 'Oui. Clés privées Ed25519 et ECDSA P-256 au format OpenSSH, y compris les clés passphrase (bcrypt-pbkdf + AES-256-CTR). Vous choisissez entre "memoriser passphrase derrière Face ID / Touch ID" ou "demander a chaque connexion" par cle.',
        },
      },
      {
        question: {
        ...
        answer: {
          en: 'Yes, by default nothing leaves your device: credentials live in the local iOS Keychain with `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`. iCloud sync of profiles is opt-in (Pro) and goes through your own private CloudKit database, with credentials end-to-end encrypted; leave it off and use the `.sshive` export/import to move profiles manually.',
          fr: 'Oui, par défaut rien ne quitte votre appareil : les credentials sont dans le Trousseau iOS local avec `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`. La sync iCloud des profils est opt-in (Pro) et passe par votre propre base CloudKit privée, credentials chiffrés de bout en bout ; laissez-la coupee et utilisez l\'export/import `.sshive` pour deplacer les profils manuellement.',
        },
      },
      {
        question: {
      ...
      },
      {
        ...
        answer: {
          en: 'No, MCP (Model Context Protocol server for Claude Code, Cursor and Claude Desktop) is macOS-only because it needs an always-on local HTTP server that iOS background restrictions prevent. Broadcast and the OTP authenticator are also Mac-only today. Everything else, SSH, SFTP, RDP, VNC, VPN, local tunnels, snippets, network tools, runs on iPhone and iPad.',
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    metaDescription: {
      ...
      fr: 'Client SFTP natif pour iPhone et iPad. Explorateur de fichiers distants, éditeur de texte intégré, file de transferts, Trousseau Touch ID. Gratuit jusqu\'a 10 Mo par fichier ; Pro illimite.',
    },
    h1: {
    ...
    hero: {
      ...
      fr: 'Naviguez sur des serveurs distants, editez les configs en place, uploadez des captures de Files en un tap. Même moteur SFTP que la version Mac, natif sur iOS.',
    },
    intro: {
      ...
      fr: 'Lire et écrire des fichiers sur un serveur distant depuis un iPhone est plus pénible que ca devrait, la plupart des workflows finissent en copier-coller dans un terminal SSH, ou avec un gestionnaire de fichiers web ouvert dans un onglet. SSHive embarque un vrai client SFTP natif sur iOS et iPadOS : un explorateur de fichiers distants, une file de transferts avec progression temps réel, un éditeur de texte intégré pour les edits sur place, et une intégration avec l\'app Files iOS pour glisser un fichier d\'iCloud Drive directement sur un serveur. Les credentials et host keys sont dans le Trousseau derrière Face ID ou Touch ID, même modèle de sécurité que le terminal SSH.\n\nFree couvre 10 Mo par fichier (suffisant pour la plupart des configs, scripts et petits assets), Pro debloque la taille illimitee et ajoute le logging de session et l\'export/import `.sshive`. Universal Purchase : payez Pro une fois sur l\'App Store, utilisez sur iPhone, iPad et Mac. Pas d\'abonnement, pas de données envoyees vers nous, et si vous tombez sur un cas d\'usage que l\'éditeur ou le browser ne couvrent pas bien, dites-le-nous, on ecoute.',
    },
    capabilities: [
      {
      ...
      {
        title: { en: 'Built-in text editor', fr: 'Éditeur de texte intégré' },
        body: {
          ...
          fr: 'Tap sur un `.conf`, `.yml`, `.json`, `.env` ou n\'importe quel fichier texte pour l\'ouvrir dans l\'éditeur de SSHive. Coloration syntaxique, numéros de ligne, recherche-remplacement, Tab/Shift+Tab indent. Save reecrit via SFTP. Parfait pour les 90 % d\'edits distants qui sont "corriger une ligne dans nginx.conf et reload".',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'SSHive est enregistre comme cible dans la share sheet iOS. Depuis Photos, Files, Mail, Messages ou n\'importe quelle app tierce, partagez vers SSHive et choisissez un dossier de destination sur n\'importe quel profil SFTP. La file de transferts gère les retries et montre la progression par fichier.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Téléchargez un fichier distant vers l\'app Files iOS (Sur mon iPhone ou iCloud Drive), y compris des selections multiples. Une fois sur iOS, AirDrop, partagez vers une autre app, ou ouvrez avec n\'importe quelle app installee. La file de download parallelise 4 transferts par défaut pour la vitesse sur les liens a forte latence.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Long-press sur n\'importe quel fichier ou dossier pour copier son chemin distant absolu dans le presse-papiers iOS, utile en paire avec le terminal SSH : collez directement dans une commande `tail -f`, `nano`, `chmod 755`. Marche pour les fichiers dans des répertoires symlinkes.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Les profils crees sur macOS peuvent être exportes en JSON `.sshive` et importes sur iOS, et inversement. Note : les credentials ne sont PAS dans l\'export (ils sont dans le Trousseau OS), uniquement host, port, username et tags. Feature Pro.',
        },
      },
    ],
    whyHeading: {
    ...
    why: [
      {
        title: { en: 'iOS Files-app integration', fr: 'Intégration Files iOS' },
        body: {
          ...
          fr: 'SSHive utilise la share sheet iOS dans les deux sens : n\'importe quoi d\'une autre app iOS peut être envoye dans SSHive pour upload, et n\'importe quel fichier distant peut être exporte vers Files / iCloud Drive / une autre app. Vos fichiers distants ne restent pas bloques dans l\'app.',
        },
      },
      {
        title: { en: 'In-place text editor', fr: 'Éditeur de texte sur place' },
        body: {
          ...
          fr: 'Pour le workflow distant le plus courant, "corriger une ligne dans une config et reload le service", l\'éditeur intégré de SSHive evite le download, edit, reupload. Tap, editez, sauvegardez. SFTP gère l\'aller-retour.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'SFTP et SSH partagent le même modèle de credentials, Trousseau iOS biometrie, pas de sync iCloud des secrets, pas de telemetrie. La même clé Ed25519 qui ouvre votre terminal SSH est celle qu\'utilise SFTP ; pas de double config.',
        },
      },
    ],
    competitorsHeading: {
    ...
    competitors: [
      {
        ...
        pitch: {
          ...
          fr: 'Un gestionnaire de fichiers polyvalent qui inclut le SFTP. Très bien si le SFTP est un outil parmi d\'autres dont vous avez besoin (cloud drives, ZIP, downloads). Pour le SFTP comme usage principal, l\'éditeur et les clés par profil de SSHive sont en général plus pratiques.',
        },
      ...
      },
      {
        ...
        pitch: {
          ...
          fr: 'Termius inclut le SFTP dans son tier abonnement. Exécution soignee ; bon choix si vous payez déjà Termius pour le SSH et voulez le SFTP dans la même app.',
        },
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: '10 Mo par fichier en gratuit, assez pour la plupart des configs, scripts et petits assets. Pro retire la limite ; upload et download sont limites uniquement par votre réseau et le disque distant.',
        },
      },
      {
        question: {
          en: 'Can I edit a remote nginx.conf from my iPhone?',
          fr: 'Puis-je éditer un nginx.conf distant depuis mon iPhone ?',
        },
        answer: {
          ...
          fr: 'Oui. Ouvrez le profil SFTP, tap sur nginx.conf, l\'éditeur intégré s\'ouvre avec coloration et numéros de ligne. Editez, tap Save, SFTP reecrit sur le serveur. Ouvrez une session SSH parallele dans la même fenêtre SSHive pour `nginx -t && nginx -s reload`.',
        },
      },
      {
        question: {
          en: 'Are downloads encrypted in transit?',
          fr: 'Les downloads sont-ils chiffrés en transit ?',
        },
        answer: {
          ...
          fr: 'Oui. SFTP est un sous-protocole de SSH, chaque octet passe par le tunnel chiffré SSH. SSHive supporte les ciphers SSH modernes : AES-256-GCM, chacha20-poly1305, avec vérification de host-key Ed25519 et ECDSA.',
        },
      },
      {
        question: {
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    intro: {
      ...
      fr: 'Microsoft Remote Desktop sur iOS est le point de depart evident, gratuit, officiel, et fait les bases. SSHive prend un angle différent : embarquer un vrai client RDP dans la même app que votre terminal SSH, SFTP, VNC et VPN, pour ne pas avoir a changer d\'app pour faire une tache ops complete. Le moteur RDP est FreeRDP 3, le même codebase moderne utilise dans beaucoup de front-ends RDP Linux, avec TLS 1.3, CredSSP, NLA et NTLMv2, se connecté a Windows Server 2016, 2019, 2022, 2025 et Windows 10/11 Pro/Enterprise sans config. NLA peut être desactive par profil pour les boxes legacy, NetBIOS et FQDN tous deux acceptés dans un champ Domaine séparé du username, et Ctrl+Alt+Suppr a son bouton dédié dans la barre RDP.\n\nLe RDP fait partie de Pro : l\'achat unique le débloque, sur iPhone, iPad et Mac a la fois. La session survit a la rotation sur iPhone Pro Max (landscape marche bien), et le client VPN intégré (IKEv2 / IPSec / OpenVPN) peut s\'auto-connecter avant le RDP pour les flottes protégées, pas besoin de jongler avec une app VPN separee. Pas d\'abonnement, pas de données envoyees vers nous. Si un coin de l\'UX RDP pourrait être plus fluide pour votre workflow, on ecoute.',
    },
    capabilities: [
      {
        ...
        body: {
          ...
          fr: 'La série FreeRDP 3 est la dernière génération du stack RDP open source, avec TLS 1.3, CredSSP, NLA et NTLMv2 supportés sans config. C\'est le même moteur largement deploye sur les bureaux Linux et embarque dans des outils tiers. SSHive le bundle nativement dans iOS sans relais cloud ni proxy, votre trafic RDP va directement de votre iPhone au serveur Windows.',
        },
      },
      {
        title: { en: 'Active Directory done right', fr: 'Active Directory bien gère' },
        body: {
          ...
          fr: 'Champ domaine séparé du username. NetBIOS (`CORP`) et FQDN (`corp.example.com`) acceptes. NLA + NTLMv2 negocies par défaut pour les environnements AD modernes. Pour les vieux DCs, desactivable par profil.',
        },
      },
      {
        title: { en: 'Configurable resolution', fr: 'Résolution configurable' },
        body: {
          ...
          fr: 'Défaut a 1920×1080 avec scaling adaptatif pour l\'écran iPhone ou iPad. Surchargeable par profil pour matcher exactement la résolution attendue par le serveur (utile quand on connecté depuis un petit iPhone et qu\'on veut que l\'écran distant rende en taille iPad côté serveur).',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Tap = clic gauche, long-press = clic droit, tap deux doigts = clic du milieu. Avec un Magic Keyboard sur iPad, toutes les touches modifier marchent. Un bouton dédié "Envoyer Ctrl+Alt+Suppr" dans la barre RDP, utile pour deverrouiller une session Windows verrouillee.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Les sessions survivent a la rotation. Sur iPhone Pro Max en landscape, le bureau Windows rend a une résolution suffisante pour être utilisable, vous pouvez ouvrir Server Manager, cliquer, taper dans une console. Pas un substitut au laptop, mais assez pour une tache admin rapide en deplacement.',
        },
      },
      {
        title: { en: 'Built-in VPN auto-connect', fr: 'Auto-connect VPN intégré' },
        body: {
          ...
          fr: 'Activez "Auto-connect VPN avant SSH/RDP" sur un profil et SSHive active le VPN configure (IKEv2 / IPSec / OpenVPN) avant de lancer la session RDP. Plus besoin de penser a ouvrir l\'app VPN d\'abord et de revenir. Utilise Apple NetworkExtension / Packet Tunnel Provider, niveau système, pas un hack sock proxy.',
        },
      },
    ],
    whyHeading: {
    ...
    why: [
      {
        ...
        body: {
          ...
          fr: 'Un champ Domaine dédié, séparé du username, qui accepte NetBIOS (`CORP`) et FQDN (`corp.example.com`). NLA + NTLMv2 negocies par defaut. Même modèle d\'entrée que les admins AD utilisent déjà sur Windows, pas besoin d\'empaqueter `DOMAIN\\username`.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Sur Microsoft Remote Desktop vous faites du RDP. Pour avoir SSH ou SFTP a côté, vous passez a une autre app et perdez le contexte VPN. SSHive garde RDP, SSH, SFTP, VNC et VPN dans une fenêtre, exactement ce qu\'il faut a un on-call a 2h du matin.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Chaque profil RDP a ses propres credentials dans le Trousseau iOS, biometrie. Microsoft Remote Desktop sauvegarde les credentials mais ne les biometrie pas par profil aussi proprement. Les entrées Trousseau de SSHive sont flagees `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`, inaccessibles si le telephone est verrouille et pas synchronisees sur iCloud.',
        },
      },
    ],
    competitorsHeading: {
    ...
    competitors: [
      ...
      },
      {
        ...
        pitch: {
          ...
          fr: 'Bureau a distance multi-protocole (RDP, VNC) avec acceleration NEAR-screen. Construit pour les deploiements équipe et entreprise.',
        },
        pricing: { en: 'Subscription', fr: 'Abonnement' },
      },
    ],
    faq: [
      ...
      },
      {
        ...
        answer: {
          ...
          fr: 'Oui. Configurez votre VPN (IKEv2, IPSec/Xauth ou OpenVPN) dans SSHive, activez "Auto-connect VPN avant SSH/RDP" sur le profil RDP, et SSHive active le VPN via Apple NetworkExtension avant d\'ouvrir la session RDP. Tailscale et WireGuard (en tant qu\'apps VPN système séparées) marchent aussi de facon transparente.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Oui, case par profil pour desactiver NLA. Utile pour les vieux Windows Server 2008 R2 ou environnements mal configures. Par défaut NLA est active, ce que les systèmes modernes attendent.',
        },
      },
      {
        question: {
          en: 'How is the keyboard handled on iPhone for Windows shortcuts?',
          fr: 'Comment le clavier est gère sur iPhone pour les raccourcis Windows ?',
        },
        answer: {
          ...
          fr: 'Une barre expose les touches que les écrans tactiles ne peuvent pas envoyer (Win, Alt, Ctrl, Esc, F1-F12, fleches). Un bouton dédié Ctrl+Alt+Suppr toujours visible. Avec un Magic Keyboard sur iPad, le clavier complet marche y compris la touche Windows (mappee a Cmd Mac par défaut, inversable par profil).',
        },
      },
    ],
    deepDiveHeading: {
    ...
    deepDive: {
      ...
      fr: 'Le RDP iOS a historiquement voulu dire "outil de depannage occasionnel", un moyen d\'atteindre un serveur Windows dix minutes puis de revenir a un vrai ordinateur. Avec les iPhone Pro Max, iPad Pro M-séries et Magic Keyboards, le plafond a bouge. Le RDP de SSHive tourne FreeRDP 3 natif (pas d\'Electron, pas de wrapper JavaScript), donc même un Server Manager ou SQL Server Management Studio bien charge reste reactif en cellulaire si le réseau serveur est rapide.\n\nEn pratique le workflow qui en beneficie le plus est la réponse a incident. Une alerte page votre telephone, vous ouvrez SSHive sur iPhone, le profil RDP sauvegarde auto-connecté le VPN, puis ouvre la VM Windows. Vous tapez dans Event Viewer, vérifiez un service, lancez une commande PowerShell. Vingt minutes plus tard vous êtes de retour au laptop. Sans SSHive ce workflow exigeait : une app VPN séparée, un client RDP séparé, des credentials AD manuellement entres dans chacun, et SSH dans encore une autre app si l\'alerte venait d\'un load balancer Linux.\n\nLa sécurité suit le même modèle que le reste de SSHive : les credentials RDP par profil dans le Trousseau iOS avec `BiometryCurrentSet`, FreeRDP tourne in-app sans relais, le client VPN utilise NetworkExtension d\'Apple (niveau système, pas un hack sock proxy), et aucune telemetrie. Le protocole RDP lui-même utilise TLS 1.3 + CredSSP, donc même si le Wi-Fi de l\'hotel est hostile, la session est protégée des sniffers passifs.\n\nLimitations a être honnete : les écrans iPhone sont petits pour du travail RDP ; les restrictions de background iOS peuvent suspendre les sessions longues si l\'appareil dort ; la sync presse-papiers entre iOS et Windows est plus contrainte que sur desktop. SSHive fait au mieux (auto-reconnect au reveil, grandes zones de tap dans la barre RDP), mais le RDP sur iPhone reste un complement au laptop, pas un remplacement. Le RDP sur iPad Pro avec Magic Keyboard est beaucoup plus proche d\'une vraie station Windows.',
    },
  },
  /* ────────────────────────────────────────────────────────────────────
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    metaDescription: {
      ...
      fr: 'Viewer VNC natif sur iPhone et iPad, moteur RoyalVNC avec TLS, ARD et RFB. Clavier VNC intégré, tap-to-click, pinch-to-zoom. Marche via tunnels SSH.',
    },
    h1: {
    ...
    intro: {
      ...
      fr: 'Le VNC sur iOS a toujours ete le cas le plus delicat, Apple Screen Sharing n\'existe pas sur iPhone, et beaucoup d\'apps VNC iOS tierces se concentrent sur le RFB plain sans TLS, sans support ARD, sans profils sauvegardes. SSHive embarque RoyalVNC sous le capot (le même moteur utilise par Royal TSX sur Mac), qui supporte toute la famille VNC : auth ARD pour Mac-vers-Mac, VeNCrypt + TLS pour RFB chiffré, et RFB nu pour serveurs legacy, enveloppe dans un tunnel SSH via l\'UI Tunnels de SSHive quand vous avez besoin d\'un chiffrement que le serveur VNC lui-même n\'offre pas.\n\nL\'UI est concue pour le tactile : tap = clic gauche, tap deux doigts = clic droit, pinch pour zoomer, pan deux doigts, et un clavier VNC a l\'écran spécifique qui expose touches de fonction, modifiers et fleches qu\'un clavier de telephone n\'a pas. Le VNC fait partie de Pro : l\'achat unique le débloque, sur iPhone, iPad et Mac a la fois. Universal Purchase avec macOS et iPad. Pas d\'abonnement, pas de données envoyees vers nous, si un serveur auquel vous vous connectez fait des siennes, dites-le-nous, on regardera.',
    },
    capabilities: [
      {
        ...
        body: {
          ...
          fr: 'Le même moteur VNC que Royal TSX, porte sur iOS et iPadOS. TLS / VeNCrypt complet, auth ARD et RFB standard. Se connecté aux serveurs RealVNC, TigerVNC, libvncserver, X11 vncserver, Partage d\'écran Apple, basiquement n\'importe quel serveur VNC raisonnable.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Tap = clic gauche, long-press = clic droit, tap deux doigts = clic du milieu. Pinch pour zoomer, pan deux doigts pour scroller un grand bureau. Tap trois doigts pour afficher/masquer le clavier VNC. Le modèle de gestes est coherent avec le reste d\'iOS, pas de mémoire musculaire propre a l\'app a reapprendre.',
        },
      },
      {
        title: { en: 'Integrated VNC keyboard', fr: 'Clavier VNC intégré' },
        body: {
          ...
          fr: 'Un clavier dédié a l\'écran expose les touches qu\'un clavier soft iOS ne peut pas envoyer : F1-F12, Esc, Tab, Ctrl, Alt, fleches, Impr écran, Inser. Combine a la saisie texte native iPhone/iPad, vous pouvez utiliser presque n\'importe quel bureau Linux/macOS sans clavier hardware.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Quand le serveur VNC supporte VeNCrypt ou TLS, SSHive le negocie. Quand il ne le supporte pas (RFB plain), utilisez l\'UI Tunnels de SSHive pour envelopper la connexion VNC dans un tunnel SSH, même fenêtre SSHive, deux clics. Chiffrement bout-en-bout même si le serveur VNC est en clair.',
        },
      },
    ],
    whyHeading: {
    ...
    why: [
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'La plupart des scenarios ops ont besoin de VNC plus autre chose, un terminal SSH pour démarrer le serveur VNC, un SFTP pour inspecter une config, un RDP vers une VM Windows sur le même reseau. SSHive bundle tout, avec profils et credentials partages.',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Les mots de passe VNC (et le username ARD si besoin) sont dans le Trousseau iOS avec `SecAccessControl` biometrique, pas de sync iCloud des secrets. Même modèle que SSH et SFTP.',
        },
      },
    ],
    competitorsHeading: {
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. ARD est un des modes d\'auth supportés par RoyalVNC. Se connecter a un Mac avec Partage d\'écran active fonctionne sans config supplémentaire, y compris avec le username et password ARD.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Oui, c\'est un des cas d\'usage les plus courants. Lancez RealVNC ou x11vnc sur le Pi, sauvegardez le profil VNC dans SSHive sur iPhone, connectez via le LAN ou un VPN domestique. Si le Pi est derrière un bastion, utilisez la feature tunnel SSH de SSHive pour forwarder le port VNC sur 127.0.0.1.',
        },
      },
      {
        question: {
          en: 'Is the VNC connection encrypted?',
          fr: 'La connexion VNC est-elle chiffrée ?',
        },
        answer: {
          ...
          fr: 'Si le serveur supporte VeNCrypt / TLS, oui, SSHive le negocie. Pour les serveurs RFB plain, utilisez l\'UI Tunnels de SSHive pour envelopper la connexion en SSH (deux clics), puis pointez le profil VNC sur 127.0.0.1. Chiffrement bout-en-bout même si le serveur VNC est en clair sur le fil.',
        },
      },
    ],
    deepDiveHeading: {
    ...
    deepDive: {
      ...
      fr: 'VNC est en fait trois protocoles dans un trench-coat : ARD (l\'extension Apple pour Partage d\'écran macOS, avec son propre handshake auth et formats de pixels), RFB VeNCrypt (TLS + auth VNC classique), et RFB plain (pas de chiffrement, juste un challenge password). La plupart des apps "client VNC" iOS ne parlent qu\'un ou deux ; le RoyalVNC de SSHive parle les trois et negocie le meilleur que le serveur offre.\n\nCa compte en pratique parce qu\'une vraie flotte est heterogene. Un mix ops typique : un Mac mini headless avec Partage d\'écran (ARD), trois bureaux Ubuntu en TigerVNC (VeNCrypt ou plain selon config), une console ESXi (variante RealVNC avec TLS strict), un Raspberry Pi en x11vnc (RFB plain). Sans client multi-protocole, il faudrait trois apps iPhone différentes pour les couvrir.\n\nLe modèle de gestes merite une note. Les clients VNC sur écrans tactiles ont historiquement ete maladroits parce que les bureaux centres souris n\'etaient pas concus pour des doigts. L\'approche de SSHive est de garder les gestes iOS-standard (tap = action principale, tap deux doigts = action secondaire, pinch pour zoomer) et d\'ajouter un overlay VNC-spécifique uniquement pour le clavier. Combine au pinch-to-zoom sur un iPhone Pro Max haute densite, on peut interagir confortablement avec un bureau Linux 1080p sans panner constamment, le geste iOS est plus ergonomique qu\'un mini pointeur souris.\n\nSecurite : VNC a une reputation d\'être non sécurisé parce que RFB plain l\'est. SSHive prend ca au serieux. L\'UI Tunnels est a un clic de n\'importe quel profil VNC, si la connexion VNC sous-jacente est en clair, on l\'enveloppe en SSH. Les credentials (mot de passe VNC, username ARD, passphrase SSH pour le tunnel d\'enveloppe) sont tous dans le Trousseau avec biometrie ; pas de fichiers style `vncpasswd` qui trainent dans le home. La vérification host key pour le wrapper tunnel SSH utilise TOFU comme le reste de SSHive, première connexion accepte, connexions futures echouent fort sur changement de cle.',
    },
  },
  /* ────────────────────────────────────────────────────────────────────
     ...
     ──────────────────────────────────────────────────────────────────── */
  {
    ...
    metaDescription: {
      ...
      fr: 'Client VPN intégré sur iPhone et iPad : IKEv2, IPSec/Xauth, OpenVPN. Auto-connect avant SSH/RDP. Apple NetworkExtension. Pas de tracking. Une app pour VPN + SSH + SFTP + RDP + VNC.',
    },
    h1: {
      en: 'A VPN client built into your SSH app',
      fr: 'Un client VPN intégré a votre app SSH',
    },
    hero: {
      en: 'IKEv2, IPSec/Xauth and OpenVPN on iPhone and iPad, auto-connected before SSH or RDP. No second app, no context switch.',
      fr: 'IKEv2, IPSec/Xauth et OpenVPN sur iPhone et iPad, auto-connecté avant SSH ou RDP. Pas de seconde app, pas de switch de contexte.',
    },
    intro: {
      ...
      fr: 'Atteindre une flotte protégée depuis un iPhone implique habituellement une app VPN d\'un vendor (Cisco AnyConnect, Pulse Secure, OpenVPN Connect) et votre app SSH/RDP d\'un autre. La danse : ouvrir l\'app VPN, s\'authentifier, switcher sur l\'app SSH, travailler, re-switcher pour deconnecter. Multipliez par membre d\'équipe et vous avez un anti-pattern ops.\n\nSSHive bundle un vrai client VPN dans l\'app iOS, IKEv2, IPSec/Xauth et OpenVPN supportés, avec import de config `.ovpn`, base sur le framework NetworkExtension / Packet Tunnel Provider d\'Apple. Activez "Auto-connect VPN avant SSH/RDP" sur un profil, et SSHive monte le VPN automatiquement avant d\'ouvrir une session, le coupe a la deconnexion. Une app, un set de credentials, pas de jonglage. Free couvre la connectivite VPN de base ; Pro ajoute profils avances, VPNs simultanés et le reste des features SSHive.',
    },
    capabilities: [
      {
        ...
        body: {
          ...
          fr: 'Les trois protocoles VPN les plus deployes supportés nativement : IKEv2 (le défaut moderne sur macOS/iOS), IPSec avec Xauth (encore courant dans les gateways corporate), et OpenVPN (via import de config `.ovpn`, les mêmes fichiers qu\'OpenVPN Connect accepte).',
        },
      },
      {
        ...
        body: {
          ...
          fr: 'Base sur le framework NetworkExtension / Packet Tunnel Provider d\'Apple, les mêmes APIs que Cisco AnyConnect et OpenVPN Connect. Le VPN est donc réel (routage L3 complet, pas un hack proxy SOCKS), respecte la gestion de batterie iOS, et apparait dans Réglages > VPN comme n\'importe quel profil VPN.',
        },
      },
      {
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Deposez un fichier `.ovpn` (d\'OpenVPN Access Server, Mullvad, configs custom ProtonVPN, n\'importe ou) dans SSHive. Le parser gère les blocs inline `<ca>`, `<cert>`, `<key>`, TLS-auth, comp-lzo, et les variantes courantes. Pas d\'étape de traduction.',
        },
      },
    ],
    whyHeading: {
    ...
    why: [
      {
        ...
        body: {
          ...
          fr: 'Le workflow ops mobile realiste est : VPN vers corporate → SSH vers bastion → SSH vers serveur interne → SFTP une config → RDP une VM Windows. SSHive est la seule app iOS qui gère chaque étape dans une fenêtre avec un setup de credentials.',
        },
      },
      {
      ...
      },
      {
        ...
        body: {
          ...
          fr: 'Le VPN de SSHive utilise NetworkExtension comme les autres apps VPN systeme. Si vous utilisez déjà Tailscale ou WireGuard pour des tailnets personnels, continuez, ils tournent a côté du VPN SSHive au niveau systeme. Le VPN SSHive couvre les cas IKEv2/IPSec/OpenVPN que Tailscale et WireGuard ne couvrent pas.',
        },
      },
    ],
    competitorsHeading: {
    ...
    competitors: [
      {
        ...
        pitch: {
          ...
          fr: 'Le client officiel OpenVPN. Solide, gratuit, OpenVPN uniquement. Il faut encore une app SSH/RDP séparée, et pas d\'orchestration auto-connect avec votre workflow remote-access.',
        },
        pricing: { en: 'Free', fr: 'Gratuit' },
      },
      {
        ...
        pitch: {
          ...
          fr: 'Le client VPN enterprise standard pour les gateways Cisco. Le choix evident quand votre entreprise utilise une infrastructure Cisco. Associez-le a SSHive pour le côté SSH/RDP, pas de conflit.',
        },
      ...
      },
      {
        ...
        pitch: {
          ...
          fr: 'VPNs mesh modernes et WireGuard-based. Excellents pour tailnets personnels et réseaux d\'equipe. Categorie différente d\'IKEv2/IPSec/OpenVPN, coexistent avec le VPN SSHive au niveau systeme.',
        },
    ...
    faq: [
      {
        ...
        answer: {
          ...
          fr: 'Oui. AirDrop ou partagez le fichier `.ovpn` dans SSHive, le parser gère les blocs inline `<ca>`, `<cert>`, `<key>`, TLS-auth et les variantes courantes. Username/password (ou passphrase de certificat client) sont ensuite demandes a la première connexion et stockés dans le Trousseau iOS.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Non. SSHive embarque zero telemetrie. La connexion VPN va directement de votre iPhone a votre endpoint configure, on ne la relaye pas par un serveur qu\'on controle. On n\'a pas accès a votre trafic VPN, vos IPs destination, ou vos credentials.',
        },
      },
      {
        question: {
        ...
        answer: {
          ...
          fr: 'Oui. Sur le profil SSH (ou RDP/VNC/SFTP), activez "Auto-connect VPN" et choisissez un profil VPN. SSHive monte le VPN avant d\'ouvrir la session et le coupe a la deconnexion. Si le VPN est déjà monte, il reutilise le tunnel existant.',
        },
      },
    ],
    deepDiveHeading: {
    ...
    deepDive: {
      ...
      fr: 'iOS ne laisse pas les apps tierces ouvrir des sockets réseau raw et pretendre être un VPN. La seule facon de faire du vrai trafic VPN sur iOS est via le framework NetworkExtension d\'Apple, specifiquement la sous-API Packet Tunnel Provider pour les tunnels style OpenVPN/WireGuard, ou le provider IKEv2/IPSec pour les protocoles qu\'Apple supporte nativement. Les apps qui utilisent NetworkExtension recoivent un entitlement special d\'Apple et tournent un processus extension séparé qui gère le tunnel chiffré ; l\'app principale parle a cette extension via IPC.\n\nSSHive fait ca proprement : l\'UI de config VPN est dans l\'app principale, le tunnel réel tourne dans un processus extension qui survit même quand l\'app principale est suspendue par iOS. C\'est pourquoi une session SSH qui utilise un VPN gère par SSHive reste connectée quand vous mettez le telephone en poche, le kernel route les paquets via l\'extension de facon transparente. A l\'inverse, un "VPN" mal construit qui tourne un proxy SOCKS dans l\'app meurt des qu\'iOS suspend l\'app.\n\nConsequence pratique : le VPN de SSHive marche pour le scenario on-call reel. La page atteint votre telephone, vous tapez un profil SSH sauvegarde avec "Auto-connect VPN" actif, le VPN monte en 1-2 secondes (plus rapide pour IKEv2, plus lent pour OpenVPN), la session SSH s\'ouvre via lui. Vous investiguez. L\'iPhone s\'endort en pleine session, le VPN reste connecté via l\'extension, le socket TCP de la session SSH survit si le réseau survit. Vous reveillez le telephone, vous êtes toujours dedans. Vingt minutes plus tard vous deconnectez ; SSHive coupe le VPN automatiquement.\n\nLe modèle de credentials continue de s\'appliquer : les credentials VPN (PSK, username/password, passphrase de certificat client OpenVPN) sont dans le Trousseau iOS avec `BiometryCurrentSet`, biometrie requise a la première connexion, puis l\'extension garde une copie transient en mémoire pendant que le tunnel est monte. Pas de profil `.mobileconfig` installe au niveau système, pas de credentials en clair sur disque.',
    },
  },
];
